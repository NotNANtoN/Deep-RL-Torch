# Adapted from https://github.com/Dotori-HJ/SelfOrganizingMap-SOM/blob/master/som.py
import torch
...
class SOM(nn.Module):
    ...
    def general_get_map_index(self):
        ''' N-dimensional mapping function'''
        idxs = []
        current_dim = 0
        for x in range(len(self.out_size)):
            current_dim_idxs = []
            for y in range(self.out_size[x]):
                current_dim_idxs.append(y)
            current_dim += 1


    def _neighborhood_fn(self, input, current_sigma):
        '''e^(-(input / sigma^2))'''
        return torch.exp(input / (current_sigma ** 2))
    ...
    def optimize(self, input, current_iter, max_iter):
        '''
